...
class ClientFoodDonationController extends AbstractController
{
    ...
    public function register(Request $request, int $id): Response
    {
        ...
        if ($this->eventRegistrationRepository->isUserRegisteredForEvent((int) $user->getId(), (int) $event->getDonationEventId())) {
            if ($isAjaxRequest) {
                ...
                return new JsonResponse([
                    ...
                    'registered_event' => $this->buildRegistrationEventPayload($event, $registrationCount),
        ...
        $smsSent = $registrationEvent->isSmsSent();
    ...
    {
        $instantRefresh = $request->query->getBoolean('instant', false);
     ...
     */
    private function buildRegistrationEventPayload(FoodDonationEvent $event, int $registrationCount): array
    {
